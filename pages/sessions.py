...
async def sessions_page():
    ...
    def get_status_badge(status: str) -> tuple[str, str]:
        ...
        mapping = {
            "done": ("Завершён", "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400"),
            "completed": ("Завершён", "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400"),
            "booked": ("Записан", "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400"),
            "abandoned": ("Прерван", "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400"),
            "transferred": ("Передан", "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400"),
            "ghost": ("Призрак", "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-400"),
            "no_slots": ("Нет слотов", "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"),
            "price_too_high": ("Дорого", "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400"),
        }
        ...
        if table_container:
            ...
            with table_container:
                ...
                with ui.row().classes("w-full bg-gray-100 dark:bg-gray-800 py-3 px-4 rounded-t-lg gap-4"):
                    ui.label("ID").classes("w-16 font-semibold text-gray-600 dark:text-gray-300 text-sm")
                    ui.label("Дата").classes("w-36 font-semibold text-gray-600 dark:text-gray-300 text-sm")
                    ui.label("Клиент").classes("w-40 font-semibold text-gray-600 dark:text-gray-300 text-sm")
                    ui.label("Статус").classes("w-32 font-semibold text-gray-600 dark:text-gray-300 text-sm")
                    ui.label("Intent").classes("w-40 font-semibold text-gray-600 dark:text-gray-300 text-sm")
                ...
                for row in data:
                    ...
                    with ui.row().classes("w-full py-3 px-4 border-b border-gray-200 dark:border-gray-700 gap-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 items-center transition-colors"):
                        ui.label(str(row.get("id", "—"))).classes("w-16 text-gray-500 dark:text-gray-400 text-sm")
                        ui.label(format_datetime(row.get("started_at"))).classes("w-36 text-gray-800 dark:text-gray-200 text-sm")
                        ui.label(client_name).classes("w-40 text-gray-800 dark:text-gray-200 font-medium text-sm")
                        
                        # Status badge
                        status_text, status_class = get_status_badge(row.get("final_status", ""))
                        ui.label(status_text).classes(f"w-32 px-2 py-1 rounded-full text-xs font-medium text-center {status_class}")
                        
                        intent = row.get("final_intent") or "—"
                        ui.label(intent).classes("w-40 text-gray-500 dark:text-gray-400 text-sm truncate").tooltip(intent)
    ...
    with page_layout("Диалоги"):
        ...
        with ui.row().classes("w-full items-center gap-4 mb-6 p-4 rounded-xl shadow-sm theme-card"):
            ...
            with ui.input("С даты").classes("w-36").props("outlined dense color=purple") as date_from:
                with ui.menu().props("no-parent-event") as menu_from:
                    ui.date().bind_value(date_from).props("minimal color=purple :locale='ru_locale'").on("input", menu_from.close).classes("w-64")
            ...
            with ui.input("По дату").classes("w-36").props("outlined dense color=purple") as date_to:
                with ui.menu().props("no-parent-event") as menu_to:
                    ui.date().bind_value(date_to).props("minimal color=purple :locale='ru_locale'").on("input", menu_to.close).classes("w-64")
            ...
            ui.button(icon="search", on_click=refresh_table).props("flat round color=purple").tooltip("Найти")
            ui.button(icon="restart_alt", on_click=reset_filters).props("flat round color=grey").tooltip("Сбросить")

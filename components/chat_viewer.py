...
def show_chat_dialog(
    ...
    ui.add_head_html("""
        <style>
            ...
            body.body--dark .chat-summary-block {
                background: linear-gradient(135deg, #3b2667 0%, #2d1f4a 100%) !important;
                border-bottom-color: #4c3575 !important;
            }
            body.body--dark .chat-summary-block .summary-text {
    ...
    with (
        ui.dialog() as dialog,
        ui.card().classes("w-[480px] max-h-[85vh] p-0 rounded-2xl overflow-hidden"),
    ):
        # Header - Telegram style
        with ui.row().classes(
            "w-full items-center justify-between lavender-header p-4"
        ):
            with ui.row().classes("items-center gap-3"):
                ...
                with ui.column().classes("gap-0"):
                    ...
                    ui.label(f"{len(history)} сообщений").classes(
        ...
        if summary:
            with ui.row().classes(
                "w-full px-4 py-3 gap-2 items-start chat-summary-block"
            ):
                ui.icon("auto_awesome").classes("text-purple-500 text-lg mt-0.5")
                ui.label(summary).classes(
                    "text-gray-700 text-sm leading-relaxed flex-1 summary-text"
                ).style("white-space: pre-wrap; word-break: break-word")

        if not history:
            with ui.column().classes(
                "items-center justify-center py-16 w-full chat-messages-area"
            ):
                ui.icon("chat_bubble_outline").classes("text-6xl text-gray-300 mb-4")
                ui.label("История сообщений не найдена").classes(
            ...
            with ui.scroll_area().classes("h-[400px] w-full p-4 chat-messages-area"):
                for msg in history:
                    ...
                    # Message bubble
                    with ui.row().classes(
                        f"w-full {'justify-end' if is_user else 'justify-start'} mb-3"
                    ):
                        bubble_class = (
                            "chat-bubble-user" if is_user else "chat-bubble-assistant"
                        )
                        with (
                            ui.card()
                            .classes(f"max-w-[85%] p-3 {bubble_class}")
                                ...
                                with ui.row().classes("gap-2 items-start"):
                                    if not is_user:
                                        ui.icon("smart_toy").classes(
                                            "text-lg text-purple-500"
                                        )
                                    ui.label(msg.get("message", "")).style(
        ...
        with (
            ui.row()
            .classes("w-full justify-end p-3 border-t")
            .style("background: var(--card-bg); border-color: var(--card-border)")
        ):
            ui.button("Закрыть", on_click=dialog.close).props(
                "unelevated rounded color=purple"
            )

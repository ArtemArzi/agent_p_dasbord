    ...
    if _supabase is None:
        ...
        if settings.debug:
            ...
            if "service_role" not in key:
                print(
                    "⚠️  WARNING: SUPABASE_SERVICE_KEY may be anon key, not service_role!"
                )
            else:
...
def get_user_by_email(email: str) -> dict | None:
    ...
    try:
        ...
        response = sb.rpc(
            "get_dashboard_user_by_email", {"user_email": email}
        ).execute()
...
def get_tenant_settings(tenant_id: str) -> dict | None:
    ...
    try:
        ...
        sb = get_supabase()
        response = (
            sb.table("tenants_v2").select("*").eq("id", tenant_id).single().execute()
        )
        if settings.debug:
            print(
                f"DEBUG get_tenant_settings: response.data = {response.data is not None}"
            )
        return response.data
    except Exception as e:
...
def update_tenant_metadata(
    ...
    if user_role != "super_admin" and user_tenant_id != tenant_id:
        print(
            f"Authorization failed: user with role '{user_role}' cannot update tenant {tenant_id}"
        )
        return False
...
def get_kpi_summary(
    tenant_id: str, date_from: str | None = None, date_to: str | None = None
) -> dict:
    ...
    try:
        ...
        # Total sessions
        sessions = (
            sb.table("conversation_sessions_v2")
...
def get_funnel_data(
    ...
    Funnel stages based on drop_off_stage in meta:
    - "started" = all sessions
    - "service_selected" = passed service selection
    - "staff_selected" = passed staff selection
    - "time_selected" = passed time selection
    - "done" = actually booked (booking_id IS NOT NULL)

    Non-booking intents (inquiry, canceled, etc.) don't count toward funnel progression.
    """
    try:
        from datetime import date, timedelta
        ...
        # Include booking_id to determine actual bookings
        result = (
            sb.table("conversation_sessions_v2")
            .select("meta, final_status, booking_id")
            .eq("tenant_id", tenant_id)
            .gte("started_at", date_from)
            .lte("started_at", date_to + "T23:59:59")
            .execute()
...
def update_wishlist_status(
    item_id: int, status: str, tenant_id: str, amount: float | None = None
) -> bool:
...
def get_wishlist_stats(tenant_id: str) -> dict:
    ...
    try:
        ...
        stats = {"converted": 0, "cancelled": 0, "pending": 0, "total_revenue": 0.0}
